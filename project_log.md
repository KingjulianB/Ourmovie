...
| 9 | Version 0.2.0, icône/logo, prêt à installer | Add-on prêt à être ajouté comme dépôt HA aujourd'hui : `config.yaml` bump à 0.2.0 (pour que HA détecte la mise à jour), `arch` étendu à amd64/aarch64/armv7, port 8099 exposé en direct en plus de l'Ingress, icône + logo générés, README/DOCS/CHANGELOG écrits. Voir `day1_objectives.md` Objectif H. |
| 10 | Deux bugs réels corrigés à l'installation (0.2.1, 0.2.2) | Conflit de port 8099 (retrait du mapping), et chemins absolus cassés sous l'Ingress + mot de passe fuité dans les logs (chemins relatifs + BASE_PATH + logs sans query string). Voir `OURMOVIE-FIX-LOG.md` et `day1_objectives.md`. |
| 11 | Extension navigateur V2 construite | Demande initiale (lien streamzo.fr) refusée — piratage. Extension générique construite à la place (contrôle `<video>` sur n'importe quel site, même backend/comptes). CORS activé, port direct 38099 (moins disputé que 8099). Vérifiée par build/type-check, pas encore testée dans un vrai navigateur. Voir `day1_objectives.md` Objectif I et `extension/README.md`. |
| 12 | Extension rejetée → app de bureau Electron construite | L'utilisateur voulait "une app qui navigue elle-même comme un navigateur, et Rave" — pas une extension. Clarifié que c'est un mur technique pour un site web (barrière navigateur), donc Electron (navigateur Chromium intégré) construit à la place. **Lancée réellement sur la machine de l'utilisateur** (pas juste compilée) — à tester par lui. Voir `day1_objectives.md` Objectif J et `desktop/README.md`. |
